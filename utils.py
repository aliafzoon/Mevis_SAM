...
def iou_torch(outputs: torch.Tensor, labels: torch.Tensor) -> float:
    SMOOTH = torch.as_tensor(1e-6, dtype=torch.float32)
...
def pixel_accuracy(pred: torch.Tensor, target: torch.Tensor) -> float:
    # Flatten the tensors to compare each pixel
...
def vis_image(
    imgs, pred_masks, gt_masks, save_path, reverse=False, points=None, thre=0.5
):
            ...
            for i in range(b):
                ...
                for pmt_id in range(p.shape[1]):
                    gt_masks[
                        i,
                        0,
                        p[i, pmt_id, 0] - 3 : p[i, pmt_id, 0] + 3,
                        p[i, pmt_id, 1] - 3 : p[i, pmt_id, 1] + 3,
                    ] = 255
                    gt_masks[
                        i,
                        1,
...
def generate_click_prompt(img, msk, pt_label=1) -> torch.Tensor:
    ...
    Returns: pt(torch.Tensor): return: prompt of shape (B, 2), prompt mask
    """
